...
public class ArtifactDetailsGrid extends AbstractGrid<ProxyArtifact, Long> {
	...
	private static final String ARTIFACT_RAW_ID_ID = "artifactRawId";
	...
	protected void addMaxColumns() {
		...
		if (isUiShowIdDefaultEnabled()) {
			getColumn(ARTIFACT_RAW_ID_ID).setHidden(false);
		} else {
			getColumn(ARTIFACT_RAW_ID_ID).setHidden(true);
		}
	}

	private boolean isUiShowIdDefaultEnabled() {
		return systemSecurityContext.runAsSystem(() -> tenantConfigManagement
				.getConfigurationValue(TenantConfigurationKey.UI_SHOW_ID_DEFAULT_ENABLED, Boolean.class).getValue());
	}

	private Column<ProxyArtifact, Long> addRawIdColumn() {
		return GridComponentBuilder.addColumn(this, ProxyArtifact::getId).setId(ARTIFACT_RAW_ID_ID)
				.setCaption(i18n.getMessage("artifact.rawid.caption"));
	}
