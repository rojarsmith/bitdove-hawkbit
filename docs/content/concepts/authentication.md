...
A Bitdove update server can be accessed in four different ways:
...
Bitdove supports multiple ways to authenticate a target against the server. The different authentication modes can be individual enabled and disabled within Bitdove. Both on system level (with Spring Boot properties) as per individual tenant.

#### Target Security Token Authentication
There is a 32 alphanumeric character security-token for each created target within IoT Bitdove. This token can be used to authenticate the target at Bitdove through the HTTP-Authorization header with the custom scheme _TargetToken_.

```
GET /SPDEMO/controller/v1/0e945f95-9117-4500-9b0a-9c6d72fa6c07 HTTP/1.1
Host: your.Bitdove.server
...
Often the targets are connected through a gateway which manages the targets directly and as a result are indirectly connected to the Bitdove update server.
...
#### Anonymous access
Here we offer general anonymous access for all targets which we consider not really sufficient for a production system but it might come in handy to get a project started in the beginning.
